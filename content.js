    ...
    if( request.message === "clicked_browser_action" ) {
      ...
      var $line_cache;
		...
		$("#ghx-chart-view").mousemove(function(e){
			...
			$("#line_h_reference").remove("");
			$("#line_v_reference").remove("");
			$("#valueHour").remove("");
			...
			var result = (((hora_result-fator_reverse)*-1)+15).toFixed(2);

			var lineHReference = $('<span id="line_h_reference" style="border: 1px solid #00FF08;width: 98%;height: 0px;position: absolute;top: '+(relY-23)+'px;left: 27px;"></span>');
            var lineVReference = $('<span id="line_v_reference" style="border: 1px solid #00FF08;width: 0px;height: 98%;position: absolute;top: 0px;left: '+(relX-42)+'px;"></span>');
			...
			$current_line = {lineReference: lineHReference,valueHour:valueHour};
		   
		    $("#ghx-chart-view").append(lineHReference);
		    $("#ghx-chart-view").append(lineVReference);
